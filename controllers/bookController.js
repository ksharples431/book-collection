...
exports.book_detail = function (req, res, next) {
  ...
  async.parallel({
    ...
    series: function (callback) {
      Series.find({ 'book': req.params.id })
        .exec(callback);
    },
  }, function (err, results) {
    if (err) { return next(err); }
    ...
    // Successful, so render.
    res.render('book_detail', { title: results.book.title, book: results.book, series: results.series });
  });
...
exports.book_delete_post = function (req, res, next) {
  ...
  async.parallel({
    book: function (callback) {
      Book.findById(req.body.id).populate('author').populate('series').exec(callback);
    },
    // book_bookinstances: function (callback) {
...
exports.book_update_get = function (req, res, next) {
  ...
  async.parallel({
    book: function (callback) {
      Book.findById(req.params.id).populate('author').populate('series').exec(callback);
    },
    authors: function (callback) {
      Author.find(callback);
    },
    ...
    //   }
    // }
    res.render('book_form', { title: 'Update Book', authors: results.authors, series: results.series, book: results.book });
  });
...
exports.book_update_post = [
  ...
  // body('series', 'Summary must not be empty.').isLength({ min: 1 }).trim(),
  

  // Sanitize fields.
  sanitizeBody('title').escape(),
  sanitizeBody('author').escape(),
  sanitizeBody('series').escape(),
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      async.parallel({
        ...
        //   }
        // }
        res.render('book_form', { title: 'Update Book', authors: results.authors, series: results.series, book: book, errors: errors.array() });
      });
      return;
    }
    else {
